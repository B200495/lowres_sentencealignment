...
for filenum in range(1, total_parts+1):
    ...
    def is_partial_match(value, tib_text, threshold=0.8):
        ...
        for i in range(len(value)):
            ...
            if len(substring) == match_length:
                if substring in tib_text:
                    return True
        return False

    #### Read entire Tibetan file into a single string, not \n-separated. ####

    with open(tib_data_path, 'r') as file:
        tib_data = file.read().replace('\n', ' ')

    # Remove commas and apostrophes from the string to maximise match chances
    tib_data = tib_data.replace(',', '').replace("'", '').lower()
    ...
    #### One dict at a time, iterate through each dict entry and search for it in the Tibetan file string. Match needs to ignore case (and punctuation?). If there's a match, keep in the dict. If no match, remove entry from dict. ####
    # Only want to include dict entries which are in the Tibetan text
    ...
    print("After Tib stop word filtering")
    print(len(dict_list[0]))
    print(len(dict_list[1]))
    print(len(dict_list[2]))
    ...
    for dictionary in dict_list:
        ...
        # Iterate through each key-value pair in the dictionary
        for key, value in dictionary.items():
        # Filter out entries that are found in the Tibetan text, normalising by removing entry punctuation to increase match chances
            if is_partial_match(key, tib_data, threshold=0.8):
                # if (" " + normalise_entry(entry) + " ") in tib_data:
    ...
    print("After Tib filtering, before Tib stopword filtering:")
    print(len(dict_list[0]))
    print(len(dict_list[1]))
    print(len(dict_list[2]))
